        ...
        @if(isset($dataTypeContent->id))
                ...
                <div class="panel" style="margin-top:10px;">
                    <div class="panel-heading new-setting">
                        <hr>
                        <h3 class="panel-title"><i class="voyager-plus"></i> Translations</h3>
                    </div>
                    <div class="panel-body">
                        <form role="form"
                              action="{{ route('admin.news.updateTranslations', $dataTypeContent->id) }}"
                              method="POST" >
                            <!-- PUT Method if we are editing -->
                        @if(isset($dataTypeContent->id))
                            {{ method_field("PUT") }}
                        ...
                        <!-- CSRF TOKEN -->
                            ...
                            <div class="panel-body">
                                ...
                                @foreach($dataTypeRows as $row)
                                    <div class="form-group">
                                        <label for="name">{{ $row->display_name }}</label>
                                        <?php $trans = $dataTypeContent->translations()->where('key',$row->field)->first()?>
                                        @if($row->type == "text")
                                            <input type="text" class="form-control" name="{{ $row->field }}"
                                                   placeholder="{{ $row->display_name }}"
                                                   value="@if(!is_null($trans))
                                                   {{ old($row->field, $trans->value) }}
                                                   @endif">
                                        @elseif($row->type == "text_area")
                                            <textarea class="form-control"
                                                      name="{{ $row->field }}">
